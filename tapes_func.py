...
def progress_bar(value, endvalue, bar_length=20):
    ...
    #sys.stdout.flush()
    # USE WITH progress_bar(x, y) and a + in the loop
    return
...
'''class Spinner:  # A simple spinner to know something is happening
...
'''def own_acmg_calculation(full_stuff, acmg_db_path, assembly, ref_anno, number_of_samples, trio_data=None,
                         pp2_percent=80, pp2_min=1, bp1_percent=10,
                         cutoff=0.005):
    # Load all databases
    ...
    PS1_df = PS1_df.set_index(['Chr', 'Start', 'Stop'])
    PS1_dict = PS1_df.to_dict(orient="index")
    ...
    if 'CLNSIG' in full_stuff.columns and 'CLNREVSTAT' in full_stuff.columns:
        ...
        print("No Clinvar annotation found; please use Clinvar annotation posterior to 2015_12_01")
        sys.exit(1)
    ...
    if 'gnomAD_exome_ALL' in full_stuff.columns and 'gnomAD_genome_ALL' in full_stuff.columns:
        ...
        print('All frequency data not found\n'
              'Please use either Exac or gnomad_exome for exome data\n'
              'and gnomad_genome for genome data\n')
        sys.exit(1)
    ...
    # PM3 requires phased parents data Not really doable
    ...
    if 'CLNSIG' in full_stuff.columns:
        ...
        print("No Clinvar annotation found; please use Clinvar annotation posterior to 2015_12_01")
        sys.exit(1)
    # BS1 Allele freq is greater than expected for disorder
    ...
    dom_list= []
    adult_list = []
    for gene, rec, dom, adult in zip(bs2_hom_het_ad_df['gene'], bs2_hom_het_ad_df['recessive'], bs2_hom_het_ad_df['dominant'], bs2_hom_het_ad_df['adult_onset']):
        if rec == 1:
    ...
    print("Done")
# TODO END OF LOADING DB
    # LIST OF DBs loaded :
    # PVS1_list
    # splice_list
    # PS1_dict list_fam
    # PS4_df
    # freq_geno_series freq_exo_series
    #  data['Domain']
    # repeat_dict
    #

# START OF COMPUTATION
    print(tmp_stmp()+"Starting ACMG criterias assignment")
    #  Starting to compute acmg criterias
    ...
    try:
        ...
        PS2_contrib_merged = [0] * len(PVS1_contrib)  # Just a list of 0's with length of first column
        print(tmp_stmp()+"No trio data, skipping PS2")
    ...
    #  Intervar probably mislabel some of those
    ...
    # BA1 frequence >0.05 Might be possible to pool it with PM2 to save time
    ...
    try:
        ...
        full_stuff['p_value'] = p_value_series.values
    except UnboundLocalError:
        pass
...
'''def check_BP3_repeat_region(repeat_dict, full_stuff, ref_anno):
    BP3_contrib = []
    chr_series = full_stuff['Chr']
    snp_start_series = full_stuff['Start']
    in_frame_series = full_stuff['ExonicFunc.{}'.format(ref_anno)]
    for chrom, snp_start, in_frame in zip(chr_series, snp_start_series, in_frame_series):
        if 'nonframeshift' in in_frame: # If nonframeshift indel
            list_of_all_first_elements = [item[0] for item in repeat_dict[
                'chr' + str(chrom)]]  # Create a list of all the starting point of repeat regions
            list_of_all_second_elements = [item[1] for item in repeat_dict[
                'chr' + str(chrom)]]  # Create a list of all the ending point of repeat regions
            ...
            range_2 = range(repeat_dict['chr' + str(chrom)][index_of_second][0],
                            repeat_dict['chr' + str(chrom)][index_of_second][1])
            # If in a repeat region AND is a nonframeshift insertion deletion or substitution
            if snp_start in range_1 or snp_start in range_2:  # If in repeat region
                BP3_contrib.append(1)
            else:
                BP3_contrib.append(0)
        else: # If Not in repeat region and not a nonframeshift
            BP3_contrib.append(0)
    return BP3_contrib'''
